...
export function Filters() {
  ...
  return (
    <div className="flex flex-wrap gap-3 rounded-xl border border-risk-border bg-risk-panel/90 p-4">
      <select className="rounded-md bg-slate-900 px-3 py-2" value={category} onChange={(e) => setCategory(e.target.value as Category)}>
        {Object.entries(categoryLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
      </select>
      <select className="rounded-md bg-slate-900 px-3 py-2" value={subcategory} onChange={(e) => setSubcategory(e.target.value)}>
        {subcategoryMap[category].map((item) => <option key={item} value={item}>{item}</option>)}
      </select>
      <select className="rounded-md bg-slate-900 px-3 py-2" value={period} onChange={(e) => setPeriod(e.target.value as Period)}>
