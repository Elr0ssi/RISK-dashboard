...
export function Filters() {
  ...
  return (
    <div className="flex flex-wrap gap-3 rounded-xl border border-risk-border bg-risk-panel/90 p-4">
      ...
      <select className="rounded-md bg-slate-900 px-3 py-2" value={mapView} onChange={(e) => setMapView(e.target.value as MapView)}><option value="dark">2D Dark</option><option value="light">2D Light</option><option value="terrain">2D Terrain</option><option value="satellite">Satellite</option></select>
      <select className="rounded-md bg-slate-900 px-3 py-2" value={theme} onChange={(e) => setTheme(e.target.value as ThemeMode)}><option value="dark">Thème sombre</option><option value="light">Thème clair</option></select>
    </div>
  );
}
